      ...
      <section className="bg-slate-900 text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <p className="text-xl text-slate-300 leading-relaxed">{t("services.lead")}</p>
          </div>
        </div>
      </section>
            ...
            {services.map((service, index) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-2xl font-bold text-slate-900 mb-3">{t(service.titleKey)}</h3>
                  <p className="text-slate-600 mb-6 leading-relaxed">{t(service.descKey)}</p>
                  <ul className="space-y-3">
                    {service.featureKeys.map((fKey, idx) => (
                      <li key={idx} className="flex items-start">
                        ...
                        <span className="text-slate-700 text-sm">{t(fKey)}</span>
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-slate-900 rounded-2xl p-12 text-center text-white">
            <h2 className="text-3xl sm:text-4xl font-bold mb-4">{t("services.custom.title")}</h2>
            <p className="text-xl text-slate-300 mb-8 max-w-2xl mx-auto">{t("services.custom.lead")}</p>
            <button className="px-8 py-4 bg-white text-slate-900 rounded-lg hover:bg-slate-100 transition-colors font-semibold text-lg">{t("services.cta")}</button>
          </div>
        </div>
      </section>
    </div>
